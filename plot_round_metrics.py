...
def plot_drift_count(df, out):
    ...
    for _,r in df.iterrows():
        ...
        if "__GLOBAL__" in items:
            # exotic case: keep backward compatibility
    ...
    plt.title("Drift flags count per round")
    plt.xlabel("Round"); plt.ylabel("# clients with drift")
    plt.grid(True, alpha=.3)
    plt.tight_layout()
...
def plot_global_flag(df, out):
    ...
    plt.step(x, y, where="post")
    plt.yticks([0,1], ["0","1"])
    plt.title("GLOBAL drift flag per round")
    plt.xlabel("Round"); plt.ylabel("Global drift flag")
    plt.grid(True, alpha=.3)
    plt.tight_layout()
...
def plot_delta_band(df, out):
    ...
    plt.fill_between(x, lo, hi, alpha=.2, label="Tolerance band")
    plt.title("Δₜ signal and tolerance band (Wilbik)")
    plt.xlabel("Round"); plt.ylabel("Δ")
    plt.legend()
    plt.grid(True, alpha=.3)
    plt.tight_layout()
...
def plot_per_client_heatmap(per_client, out):
    ...
    plt.title("Heatmap: accuracy by client × round")
...
def plot_per_client_mean(per_client, out):
    ...
    plt.xlabel("Mean accuracy"); plt.ylabel("Client")
    plt.title("Mean accuracy per client")
    plt.tight_layout()
...
def main():
    ...
    ap.add_argument("--csv", help="results/<run>/round_metrics.csv")
    ap.add_argument("--per-client-csv", default=None)
    ap.add_argument("--results_dir", default=None, help="folder containing the CSVs (alternative to the args above)")
